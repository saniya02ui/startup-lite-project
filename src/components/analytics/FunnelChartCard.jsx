...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900 text-slate-100 p-3.5 rounded-2xl shadow-xl border border-slate-700 text-xs space-y-1.5">
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col h-[400px]">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500">Pipeline progression and stage drop-off metrics</p>
      </div>
        ...
        <div className="w-full sm:w-1/2 overflow-y-auto max-h-[240px] pr-2 space-y-2.5">
          {data.map((item, idx) => {
            ...
            return (
              <div 
                key={item.name}
                className="p-3 bg-gray-50 dark:bg-gray-900/35 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col gap-1.5"
              >
